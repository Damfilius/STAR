...
/*
pieceStartIn - starting position of the read from which we are mapping
pieceLengthIn - remaining length of the read to be mapped  
iDir - direction with which we are mapping (left or right)
iSA1 - 0
iSA2 - genome length
maxLbest - max mappable length 
iFrag - split fragment count
...
uint ReadAlign::maxMappableLength2strands(uint pieceStartIn, uint pieceLengthIn, uint iDir, uint iSA1, uint iSA2, uint& maxLbest, uint iFrag) {
    ...
    /*
    * gSAindexNbases - the length of the pre-indexed strings by default
    */

    /*
    ! LOOP ITERATING OVER THE LENGTH OF THE PIECE TO BE MAPPED
    ! ind1 IS THE KEY TO THE PRE-INDEX
    */
    for (uint iDist=0; iDist < min(pieceLengthIn, P.pGe.gSAsparseD); iDist++) {//cycle through different distances
        uint pieceStart; // position from whcih to start mapping the piece
        uint pieceLength = pieceLengthIn - iDist; // length of the piece we are trying to map

        // calculate full index
        uint Lmax = min(P.pGe.gSAindexNbases, pieceLength); // length of string we will be searching for in the pre-indexed string
        uint ind1 = 0;
        if (dirR) {//forward search
            pieceStart = pieceStartIn + iDist; // starting position within the read from where we are checking the pre-index 
            //! read[pieceStart:Lmax] - this is what we are looking for in the pre-index
            for (uint ii=0; ii < Lmax; ii++) {//calculate index TODO: make the index calculation once for the whole read and store it
                ind1 <<=2LLU; // ind1 *= 4 (shift by 2 bits)
                ind1 += ((uint) Read1[0][pieceStart+ii]); //? WHAT IS Read1[0][pieceStart+ii]
            };
        } else {//reverse search
        ...
        /*
        !iSA1 AND iSA2 define the interval within the suffix array that we will be searching
        */


        //find SA boundaries - the interval within which we will perform the binary search
        //! WE SUCCESSIVELY DECREASE THE STRING WE ARE SEARCHING FOR IN THE PREINDEX IF WE CANNOT FIND IT
        uint Lind=Lmax;
        while (Lind>0) {//check the presence of the prefix for Lind
            ...
            if ((iSA1 & mapGen.SAiMarkAbsentMaskC) == 0) {//prefix exists
                break;
            } else {//this prefix does not exist, reduce Lind - decrement the string you are searching for in the pre-index
                --Lind;
                ind1 = ind1 >> 2;
            };
        };
    ...
    //#define SA_SEARCH_FULL
        ...
        if (Lind < P.pGe.gSAindexNbases && iSA1noN && iSA2good) {//no need for SA search
            ...
            maxL=Lind;
        /*
        ! the interval consists of only 1 entry - there is only one such prefix in the genome
        */
